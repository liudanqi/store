...
class Seller():
    ...
    def add_book(self, uid, sid, info, stock_level):
        ...
        db.stock.update({'id': info.id}, {'$push': {'owner': uid}})
    ...
    def add_stock_level(self, uid,sid,bid,add_stock_level):
        ...
        session = DBSession()
        e_sid = session.query(Store).filter(Store.sid == sid)
        e_bid = db.stock.find({'id': bid, 'owner': uid})
        if e_sid.count() == 0:
            session.commit()
            session.close()
            return 502   # 商铺ID不存在
