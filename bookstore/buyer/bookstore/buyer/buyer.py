...
from buyer.table import Store,User,Depository,Order,Stock
...
class Buyer():
    ...
    def new_order(self, store_id, books):
        ...
        session = DBSession()
        try:
            session.query(User).filter(User.uid == self.user_id)
        except:
            session.close()
            return 511, "买家用户ID不存在"         #non exist user id
        try:
            session.query(Store).filter(Store.sid == store_id)
        except:
            session.close()
        ...
        for item in books:
            id = item[0]
            bookcount = item[1]
            book = session.query(Depository).filter(Depository.store_id == store_id, Depository.bid == id).one()
            if book != None:
        ...
        #neworder = Order(order_id,self.user_id,store_id,total,0)
    ...
    def payment(self, order_id):
        ...
        if total > money:
            session.close()
            return 598
        elif user.pay_pwd != self.password:
            session.close()
            return 597
        else:
